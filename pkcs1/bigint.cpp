...
MontgomeryReductionContext::MontgomeryReductionContext()
	: datablock(0), size(0), allocsize(0)
{}

MontgomeryReductionContext::~MontgomeryReductionContext()
{
	allocsize = 0; // just in case
	if (this->datablock)
...
void MontgomeryReductionContext::Prepare(const uint8_t * pn, unsigned nlen, unsigned vlen, bool netByteOrder)
{
	//1) reallocate common buffer
	// - if needed
	if (this->allocsize < vlen) {
		if (this->datablock)
			delete[] this->datablock;

		this->datablock = new uint32_t[TOTAL_CONTEXT_SIZE(vlen)];
		this->allocsize = vlen;
	}
