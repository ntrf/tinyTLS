...
extern int GetRSAAlgorithmType(const uint8_t * oid, uint32_t length);

// ### check for non-rsa algorithms
void ExtractAlgorithmId(int & algo, ASNElement & el)
{
	// SEQUENCE
	//   OBJID --algoid
	//   NULL --params
			...
			if (type.compare(ExtBasicContraints, sizeof(ExtBasicContraints)) == 0) {
				...
				if (isca < end) {
					...
					if (_isca != 0) {
						...
						out->chainLength = INT32_MAX; // no limit by default

						ASNElement chainlimit = isca.next();
						if (chainlimit < end) {
							out->chainLength = chainlimit.getInt();
						}
					}
				}
...
//     publicExponent INTEGER -- e }
	...
	out->exponent.alloc(exponent.length());
	exponent.extract(out->exponent.data);
